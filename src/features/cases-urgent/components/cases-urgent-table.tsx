...
import { getCasesUrgentColumns } from './cases-urgent-columns'
...
export function CasesUrgentTable(_: DataTableProps) {
  ...
  const { data: ownerAllRows = [] } = useQuery({
    queryKey: ['owner-picker-all-for-case-urgent-list'],
  ...
  const { data: serviceContactAllRows = [] } = useQuery({
    queryKey: ['contact-all-for-case-urgent-list'],
  ...
  const { data: divisionCollabRows = [] } = useQuery({
    queryKey: ['division-collab-all-for-case-urgent-list'],
  ...
  const { data: supplierAllRows = [] } = useQuery({
    queryKey: ['supplier-all-for-case-urgent-list'],
  ...
  const { data: vesselAllRows = [] } = useQuery({
    queryKey: ['vessel-picker-all-for-case-urgent-list'],
        ...
        <div className='flex items-center gap-2'>
          <Button
            ...
            onClick={async () => {
              ...
              await queryClient.refetchQueries({
                queryKey: ['case-urgent-list-groups'],
              })
            }}
          >
