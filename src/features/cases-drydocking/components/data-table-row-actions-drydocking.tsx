...
import { deleteCase } from '../../cases/api/client'
import { buildCaseNavSearch } from '../../cases/api/nav-helpers'
import { type Case } from '../../cases/data/schema'

const route = getRouteApi('/_authenticated/case_drydocking_list/')

type DataTableRowActionsDrydockingProps = {
  row: Row<Case>
}

export function DataTableRowActionsDrydocking({ row }: DataTableRowActionsDrydockingProps) {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: (ok) => {
      if (ok) {
        toast.success('案件已删除')
        queryClient.invalidateQueries({ queryKey: ['case-drydocking-list-paginated'] })
