...
export function CasesDrydockingTable(_: DataTableProps) {
  ...
  const { data: supplierAllRows = [] } = useQuery({
    queryKey: ['supplier-picker-all-for-case-list'],
  ...
  const supplierIdNameMap = useMemo<Map<number, string>>(() => {
    ...
    for (const s of list) {
      ...
      const name =
        String((s as any).supplier_name ?? '').trim() ||
  ...
  const { data: vesselAllRows = [] } = useQuery({
    queryKey: ['vessel-picker-all-for-case-drydocking-list'],
  ...
  const enforcedDryDockingValues = useMemo<string[]>(() => {
    const out: string[] = []
    for (const o of inqTypeAOptions) {
      if (!o.value) continue
      ...
      const match =
        lblUp.includes('DRY-DOCKING') ||
        lbl.includes('坞修') ||
        valUp.includes('A-DRY')
      if (match) out.push(val)
    }
    return out
  }, [inqTypeAOptions])
  ...
  const {
    ...
    queryKey: [
      'case-drydocking-list-paginated',
      ...
      enforcedDryDockingValues,
      vesselNamesFilter,
    ],
    queryFn: () => {
      const baseInquiryType: string[] =
        enforcedDryDockingValues.length > 0 ? [...enforcedDryDockingValues] : []
      let mergedInquiryType = baseInquiryType
      if (caseInquiryTypeFilter.length > 0) {
        const s1 = new Set(baseInquiryType.map((x) => String(x).toUpperCase()))
        mergedInquiryType = caseInquiryTypeFilter.filter((x) =>
          s1.has(String(x).toUpperCase())
        )
      }
      return fetchCasePaginated({
        ...
        caseShouldHandleToday:
          caseShouldHandleTodayFilter.length > 0
            ...
            : undefined,
        caseInquiryType:
          mergedInquiryType.length > 0 ? mergedInquiryType : undefined,
        caseIncharge:
  ...
  const memoQuery = useQuery({
    queryKey: [
      'case-drydocking-memo-list-by-page-case-ids',
      currentPageCaseIds
        .slice()
        .sort((a, b) => a - b)
        .join(','),
  ...
  const inquiryQuery = useQuery({
    queryKey: [
      'case-drydocking-inquiry-list-by-page-case-ids',
      currentPageCaseIds
        .slice()
        .sort((a, b) => a - b)
        .join(','),
  ...
  useEffect(() => {
    ...
    for (const inquiry of rows) {
      const rawId = (inquiry as any).case_id
      const cid =
        typeof rawId === 'number'
          ? rawId
          : typeof rawId === 'string' && rawId.trim() !== ''
          ...
          <Button
            ...
            onClick={async () => {
              await queryClient.refetchQueries({
                queryKey: ['case-drydocking-list-paginated'],
              })
              await queryClient.refetchQueries({
                queryKey: ['case-drydocking-list-groups'],
              })
            }}
          >
