...
export type AgentContactPickerResult = {
  contact_id: string
  contact_name: string
...
export type AgentContactPickerDialogProps = {
  open: boolean
  onOpenChange: (open: boolean) => void
  onSelect: (result: AgentContactPickerResult) => void
  initialSelectedName?: string
}

const AGENT_CONTACT_TYPE = 'J1'
...
export function AgentContactPickerDialog({
  open,
  onOpenChange,
  onSelect,
  initialSelectedName,
}: AgentContactPickerDialogProps) {
  ...
  const contacts = useMemo<Contact[]>(() => {
    return (allContacts as Contact[]).filter(
      (c) =>
        String(c.contact_type ?? '').toUpperCase() === AGENT_CONTACT_TYPE
    )
  }, [allContacts])
  ...
  const agentContactTypeLabel = useMemo(() => {
    return (
      resolveLabel(AGENT_CONTACT_TYPE, typeMap) ||
      AGENT_CONTACT_TYPE
    )
  }, [typeMap])
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className='sm:max-w-[1200px]'>
        <DialogHeader>
          <DialogTitle>选择案件代理</DialogTitle>
          <DialogDescription>
            从 {agentContactTypeLabel}（{AGENT_CONTACT_TYPE}）类型联系人列表中选择作为案件代理，支持关键词搜索、列排序、行点击快速选择。
          </DialogDescription>
        </DialogHeader>
        <div className='flex flex-col gap-3'>
          <div className='flex flex-wrap items-center gap-2'>
            <div className='relative w-[420px] min-w-[360px]'>
              ...
              <Input
                placeholder='按案件代理姓名 / 职级 / 业务类型 / 手机 / 邮箱 / 备注搜索...'
                value={searchKeyword}
                  ...
                  {contactsLoading ? (
                    ...
                    <TableRow>
                      ...
                      >
                        {searchKeyword.trim() !== ''
                          ? '未找到匹配的案件代理，请更换搜索关键词。'
                          : `暂无${agentContactTypeLabel}（${AGENT_CONTACT_TYPE}）的数据。`}
                      </TableCell>
                    </TableRow>
                  ) : (
