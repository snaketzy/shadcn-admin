...
import { getCasesServiceColumns } from './cases-service-columns'
...
export function CasesServiceTable(_: DataTableProps) {
  ...
  const { data: ownerAllRows = [] } = useQuery({
    queryKey: ['owner-picker-all-for-case-service-list'],
  ...
  const { data: serviceContactAllRows = [] } = useQuery({
    queryKey: ['contact-all-for-case-service-table'],
  ...
  const { data: divisionCollabRows = [] } = useQuery({
    queryKey: ['division-collaborations-for-case-service-table'],
  ...
  const { data: divisionKRowsData = [] } = useQuery({
    queryKey: ['case-dict-prefix-K-service-table'],
  ...
  const { data: supplierAllRows = [] } = useQuery({
    queryKey: ['supplier-all-for-case-service-list'],
  ...
  const { data: vesselAllRows = [] } = useQuery({
    queryKey: ['vessel-picker-all-for-case-service-list'],
  ...
  const memoQuery = useQuery({
    queryKey: [
      'case-service-memo-list-by-page-case-ids',
      currentPageCaseIds
        .slice()
        .sort((a, b) => a - b)
        .join(','),
  ...
  const inquiryQuery = useQuery({
    queryKey: [
      'case-service-inquiry-list-by-page-case-ids',
      currentPageCaseIds
        .slice()
        .sort((a, b) => a - b)
        .join(','),
          ...
          <Button
            ...
            onClick={async () => {
              await queryClient.refetchQueries({
                queryKey: ['case-service-list-paginated'],
              })
              await queryClient.refetchQueries({
                queryKey: ['case-service-list-groups'],
              })
            }}
          >
