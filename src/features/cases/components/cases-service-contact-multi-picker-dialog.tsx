...
export type CasesServiceContactMultiPickerResult = {
  contact_ids: number[]
  contact_names: string[]
  items: Contact[]
}

export type CasesServiceContactMultiPickerDialogProps = {
  open: boolean
  onOpenChange: (open: boolean) => void
  onSelect: (result: CasesServiceContactMultiPickerResult) => void
  initialSelectedIds?: string[]
}

export function CasesServiceContactMultiPickerDialog({
  open,
  onOpenChange,
  onSelect,
  initialSelectedIds,
}: CasesServiceContactMultiPickerDialogProps) {
  ...
  useEffect(() => {
    if (!open) return
    setSearchKeyword('')
    const next = new Set<string>(initialSelectedIds ?? [])
    setSelectedIds((prev) => {
  ...
  const handleConfirm = useCallback(() => {
    ...
    const pickedItems: Contact[] = []
    const pickedIds: number[] = []
    const pickedNames: string[] = []
    for (const c of contactList) {
      const idStr = String(c.contact_id)
      if (selectedIds.has(idStr)) {
        pickedItems.push(c)
        const idNum = Number(c.contact_id)
        if (Number.isFinite(idNum) && idNum > 0) {
          pickedIds.push(idNum)
        }
        pickedNames.push(c.contact_name ?? '')
      }
    }
    // Keep order per selectedIds set iteration
    const orderedItems: Contact[] = []
    const orderedIds: number[] = []
    const orderedNames: string[] = []
    for (const idStr of idList) {
      const found = pickedItems.find(
        (c) => String(c.contact_id) === idStr
      )
      if (found) {
        orderedItems.push(found)
        const idNum = Number(found.contact_id)
        if (Number.isFinite(idNum) && idNum > 0) {
          orderedIds.push(idNum)
        }
        orderedNames.push(found.contact_name ?? '')
      }
    }
    onSelectRef.current({
      contact_ids: orderedIds,
      contact_names: orderedNames.filter(Boolean),
      items: orderedItems,
    })
  ...
  const columns = useMemo<ColumnDef<Contact, unknown>[]>(() => {
    return [
      ...
      },
      {
        ...
        meta: {
          className: cn(
            'sticky z-20 w-[200px] min-w-[200px] bg-background ps-0.5',
            'shadow-[inset_-1px_0_0_hsl(var(--border))]'
          ),
          thClassName: cn(
            'sticky top-0 z-40 w-[200px] min-w-[200px] bg-background ps-0.5',
            'shadow-[inset_-1px_0_0_hsl(var(--border))]'
          ),
        },
        enableHiding: false,
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className='sm:max-w-[1200px]'>
        <DialogHeader>
          <DialogTitle>选择服务负责人</DialogTitle>
          <DialogDescription>
            从联系人列表中选择服务负责人（可多选），支持关键词搜索、列排序、行点击勾选。
          </DialogDescription>
        </DialogHeader>
        <div className='flex flex-col gap-3'>
          <div className='flex flex-wrap items-center gap-2'>
            <div className='relative w-[420px] min-w-[360px]'>
              ...
              <Input
                placeholder='按姓名 / 类型 / 单位 / 职级 / 手机 / 邮箱 / 备注搜索...'
                value={searchKeyword}
