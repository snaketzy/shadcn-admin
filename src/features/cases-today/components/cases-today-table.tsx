...
import { getCasesTodayColumns } from './cases-today-columns'
...
export function CasesTodayTable(_: DataTableProps) {
  ...
  const { data: serviceContactAllRows = [] } = useQuery({
    queryKey: ['contact-all-for-case-today-list'],
  ...
  const { data: divisionCollabRows = [] } = useQuery({
    queryKey: ['division-collab-all-for-case-today-list'],
  ...
  const { data: supplierAllRows = [] } = useQuery({
    queryKey: ['supplier-all-for-case-today-list'],
  ...
  const enforcedHandleTodayValues = useMemo<string[]>(() => {
    return Array.from(handleTodayBIsYesSet)
  }, [handleTodayBIsYesSet])
  ...
  const {
    ...
    queryFn: () => {
      const baseHandleToday: string[] =
        enforcedHandleTodayValues.length > 0 ? [...enforcedHandleTodayValues] : []
      let mergedHandleToday = baseHandleToday
      if (caseShouldHandleTodayFilter.length > 0) {
        const s1 = new Set(baseHandleToday.map((x) => String(x).toUpperCase()))
        mergedHandleToday = caseShouldHandleTodayFilter.filter((x) =>
      ...
      return fetchCasePaginated({
        ...
        caseShouldHandleToday:
          mergedHandleToday.length > 0 ? mergedHandleToday : undefined,
        caseInquiryType:
  ...
  const memoQuery = useQuery({
    queryKey: [
      'case-today-memo-list-by-page-case-ids',
      currentPageCaseIds
        .slice()
        .sort((a, b) => a - b)
        .join(','),
  ...
  const inquiryQuery = useQuery({
    queryKey: [
      'case-today-inquiry-list-by-page-case-ids',
      currentPageCaseIds
        .slice()
        .sort((a, b) => a - b)
        .join(','),
